...
export function ProductCard({ product }: { product: Product }) {
  ...
  const seller = getSellerByName(product.brand)
  return <article className="group relative flex flex-col">
    ...
    <div className="flex items-start justify-between gap-3">
      <div>{seller ? <Link href={`/marcas/${seller.slug}`} className="mb-1 block text-[10px] font-bold uppercase tracking-[0.16em] text-[#9b7766] hover:text-[#b65f3d]">{product.brand}</Link> : <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.16em] text-[#9b7766]">{product.brand}</p>}<Link href={`/products/${product.id}`} className="font-serif text-lg text-[#2d211c] hover:text-[#9b4b2d]">{product.name}</Link><p className="mt-1 text-sm text-[#86756e]">{product.notes}</p></div>
